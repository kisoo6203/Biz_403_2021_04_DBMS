...
-- Oracle에서는 SYSDBA 권한등급이 있으며
--  일반적인 DBA보다 높은 권한을 갖고 있다
--  SYSDBA 권한은TableSpace를 만들고, User를 만드는 역할
-- SYS 사용자로 접속한 상태

-- CREATE TABLESPACE schoolDB DATAFILE SIZE 1M AUTOEXTEND ON NEXT 1K;
...
-- 새로운 사용자 계정을 등록할 때 TableSpace와 연결을 하여
-- 데이터를 추가할 수 있도록 설정
...
GRANT CREATE SESSION TO scuser; -- create session 권한을 준다 : grant
-- REVOKE ** FROM user : user에게서 **권한을 제거하는것
...
-- 원칙은 사용자에게 권한을 일일히 지정하여 
--      세부적으로 관리해야 한다.
--      하지만 학습(실습)하는 입장에서
--      일일히 세부적으로 지정하기는 매우 불편함이 있다
--      사용자등록을하고 DBA(DB관리자) 권한을 부여하여 
--      실습에 용이하도록 사용하겠다
GRANT DBA TO scUSER; -- 모든권한을 주는것
