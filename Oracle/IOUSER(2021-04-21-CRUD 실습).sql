...
-- 데이터 전체 조회
SELECT io_date, io_buyer, io_pname, io_price, io_total
FROM tbl_iolist;
...
-- projection 지정
SELECT io_pname, io_buyer FROM tbl_iolist; -- 순서바꾸기
...
-- 3. 리스트를 출력한다
SELECT io_date AS 거래일자, -- AS : 이름 집어넣을때 사용
        io_buyer AS 고객명,
        io_pname AS 상품명,
        io_qty AS 수량,
        io_price AS 단가, 
        io_qty * io_price AS합계
        FROM tbl_iolist; -- iolistDB.iouser.tbl_iolist에서 데이터를 가져와라
                -- iolistDb에서 iouser가 만든 tbl_iolist에서 ...
...
-- 2. 가져온 데이터 중에서 io_buyer칼럼에 저장된 값이
--      '홍길동' 인 데이터만 간추려서 
-- 3. 보여달라
SELECT *
FROM tbl_iolist
WHERE io_buyer = '홍길동'; -- 내가 찾고있는 데이터가 어디있나?where

-- tbl_iolist에 저장되어 있는 데이터 리스트중에서
-- io_buyer 칼럼의 값이 '홍길동' 인 데이터만 간추려서
-- io_buyer, io_pname 칼럼만 보이고 나머지 칼럼은 숨겨서
-- 보여달라
