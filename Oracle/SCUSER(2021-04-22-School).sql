...
CREATE TABLE tbl_student(
    ...
    st_tel VARCHAR(20), -- 000-0000-0000 13자리이지만 여유를 줌
    st_addr nVARCHAR2(125) 
);

-- 생성한 Table에 데이터 추가
-- DML(데이터조작어) 명령어를 사용하여 데이터 추가(create)
...
SELECT * FROM tbl_student; -- 현재 학번이 같고 둘째행에 이름값이 비어있음
...
CREATE TABLE tbl_student(
    st_num CHAR(5) UNIQUE NOT NULL, -- 중복금지, 비어있으면 안됨
    st_name nVARCHAR2(20) NOT NULL, -- 비어있으면 안됨
    st_dept nVARCHAR2(10),
    st_grade VARCHAR(5), -- 숫자값을 입력, 숫자값을 문자형으로 인식
    st_tel VARCHAR(20), -- 000-0000-0000 13자리이지만 여유를 줌
    st_addr nVARCHAR2(125) 
...
-- 데이터를 추가하려고하니 문제가 있어서 INSERT 불가
-- Table의 제약조건을 설정할때
...
-- PRIMARY KEY : UNIQUE + NOT NULL + 기타조건 + INDEX 자동생성
--   매우 강력한, 가장 우선순위가 높은 제약조건이다.
DROP TABLE tbl_student;
CREATE TABLE tbl_student(
    st_num CHAR(5) PRIMARY KEY, -- UNIQUE, NOT NULL, 기타조건
...
-- PK(primary key)로 설정된 칼럼에 조건을 부여하여 데이터 조회하기
SELECT *
FROM tbl_student
WHERE st_num = '00001'; -- 00001이 담긴 칼럼을 알려달라
