...
--  데이터를 추가, 조회, 수정, 삭제 실습을 한다

-- TABLE
--  DBMS를 사용하여 데이터를 추가하면
--  마치 execl의 sheet에 데이터를 저장하는 것처럼
--  데이터가 저장된다.
--  실제 DBMS소프트웨어가 저장하는 방식은
--        고유한 기술이 적용되어 있지만
--  DBMS 소프트웨어를 사용하는 사용자는 모든 데이터는
--      표(TABLE) 형식으로 저장된 것으로 인식하는 표준 형식이다
-- Oracle의 데이터 저장 구조 : TableSpacs > Table > 데이터들
-- TABLE은 Java 코딩에서 VO클래스를 만드는 원리와 비슷하다
...
CREATE TABLE tbl_score (
    sc_num cHAR(5) , -- 문자열 5자리의 데이터만 저장, String num (Java에서)
...
--       Data Management Lang, 데이터관리
-- 생성된 TABLE에 데이터추가, 조회, 수정, 삭제를 하는 명령어
-- DBMS 대표적인 CRUD 업무
--   데이터추가(데이터 생성 : create 업무) : INSERT 명령어사용
--   데이터조회(읽기 등등 : Read) : SELECT 명령어사용
--   데이터변경, 수정 : UPDATE 명령어 사용
--   데이터삭제 : DELETE 라는 명령어 사용
...
UPDATE tbl_score
SET sc_name = '임꺽정'; -- 아주 매우 주의해야할 코딩 !!!!!!!!!!!!!!!!!!!!!!
