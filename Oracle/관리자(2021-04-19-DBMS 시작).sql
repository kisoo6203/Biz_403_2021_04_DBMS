...
CREATE TABLESPACE mydb  -- 저장소 만들기
DATAFILE 'C:/oraclexe/data/mydb.dbf' -- 호환성을위해 \ 를 /로
SiZE 10M AUTOEXTEND ON NEXT 10K; -- 10메가바이트로 만들기 공간이부족하면 10키로바이트씩 자동으로 늘려줌

-- SQL Developer에서 명령 실행하기
-- SQL 코드를 입력하고 명령줄내에서 Ctrl + Enter를 입력
--  가. SQL Developer는 localhost:1521에서 기다리고 있는
--      Oracle DBMS서버에게 명령을 전달한다
--  나. Oracle DMS 서버는 명령을 수신한 후 문법검사를 하고
--      명령을 실행한 후 결과를 다시 SQL Developer에게 return
--  다. SQL Developer는 Console에 결과를 보여준다

-- DROP : CREATE로 생성한 객체(Object, 요소)를 제거하는 명령
...
-- USER(사용자 계정)
-- 일반적인 DBMS 소프트웨어에서는 사용자 계정은 DBMS서버에 접속하기 위한
--      역할만 수행한다
-- Oracle에서의 사용자 계정은 데이터 저장소와 연결하는
--      독특한 역할도 수행한다.
-- A라는 사용자가 생성한, 연결된 TableSpace가 있다면
--      다른 사용자는 TableSpace에 데이터를 추가, 수정, 삭제, 조회
--      하는데 제한을 둘 수 있다.
-- 사용자를 등록할 때 사용자만의 TableSpace를 지정하여 줄 수 있다
